...
class DES {
    private:
        ...
        string intialize(string pt, string type){
            string plaintext;
            if (type=="hex"){ 
                if (pt.length()!=16) throw invalid_argument("Key and plaintext size must be 16 characters in hexadecimal format.");
                for (int i=0; i<16; i++) plaintext+=hex_to_bin(pt[i]);
            }
            else if (type=="string"){
                if (pt.length()>8) throw invalid_argument("Key and plaintext size must be 8 characters minimum in string format.");
                stringstream ss;
        ...
        string apply_initial_permutation(string pt, string type){
            pt = intialize(pt, type);
            string res;
        ...
        void key_generation(string ky){
            keys.clear();
            if (ky.length()!=16) throw invalid_argument("Key and plaintext size must be 16 characters in hexadecimal format.");
            string key;
        ...
        string crypt(string pt, string ky, bool is_encrypting, string type){
            string plaintext = apply_initial_permutation(pt, type);
    ...
    public:
        ...
        string encrypt(string pt, string ky, string type="hex"){
            return crypt(pt, ky, true, type);
        }
        string decrypt(string pt, string ky){
            return crypt(pt, ky, false, "hex");
        }
};
...
int main(){
    ...
    cout << "DES Encryption/Decryption [only up to 8 characters OR HEX representations (16 characters)]\n";
    string choice;
    cout << "Encrypt [e] or Decrypt[d]: ";
    getline(cin, choice);
    if (choice.length()==0) exit(1);
    if (tolower(choice[0])=='e'){
        cout << "Enter text to encrypt: ";
        getline(cin, text);
        cout << "Enter key (in HEX): ";
        getline(cin, key);
        cout << "Enter mode of text (Hex, String): ";
        getline(cin, mode);
        cout << "= " << des.encrypt(text, key, (tolower(mode[0])=='s') ? "string" : "hex") << endl;
    } else {
        cout << "Enter text to decrypt: ";
        getline(cin, text);
        cout << "Enter key (in HEX): ";
        getline(cin, key);
        cout << "= " << des.decrypt(text, key) << endl;
    }
}